...
Page({
  ...
  //监听页面显示
  onShow: function () {
    let phone= wx.getStorageSync('phone')? wx.getStorageSync('phone'): this.data.phone
    this.setData({phone})
  },
  // 填写信息绑定-------------------------------------------------------------
  bindName(e){
  ...
  toSubmit(){
    ...
    if(!this.data.latitude){
      wx.showToast({title: '请选择定位位置',icon: 'none'})
      return
    }
    if(!this.data.code){
    ...
    if(!this.data.flag){
      wx.showToast({title: '请先阅读商家入驻协议并勾选同意',icon: 'none'})
      return
    }
